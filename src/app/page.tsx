  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-8 right-8 z-200 w-11.5 h-11.5 rounded-full border border-(--border) bg-(--ink2) text-gold flex items-center justify-center text-lg transition-all duration-300 backdrop-blur-[10px] cursor-none"
      ...
      <section id="hero" className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute left-16 top-0 bottom-0 w-px hero-side-line" />
        <div className="absolute right-16 top-0 bottom-0 w-px hero-side-line" />
        ...
        >
          <motion.div
            ...
            className="flex items-center justify-center gap-4 font-mono text-[0.72rem] tracking-[0.25em] uppercase text-gold mb-6"
          >
            <span className="inline-block w-10 h-px bg-gold opacity-50" />
          ...
          <motion.h1
            ...
            className="heading-font font-light leading-[0.88] tracking-[-0.02em] text-(--cream) mb-8 text-[clamp(5rem,12vw,11rem)]"
          >
            Rahul<br />
          ...
          <motion.p
            ...
            className="text-[0.95rem] text-(--muted) tracking-[0.05em] mb-12 leading-loose"
          >
        ...
        <div className="scroll-hint absolute bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 font-mono text-[0.65rem] tracking-[0.25em] uppercase text-(--muted)">
          <span>Scroll</span>
          <div className="scroll-line w-px h-12.5 bg-linear-to-b from-gold to-transparent" />
        </div>
      </section>

      {/* ══════════════ MARQUEE ══════════════ */}
      <div className="overflow-hidden py-6 border-t border-b border-(--border) bg-(--ink3)">
      ...
      <section
        id="about"
        className="py-40 px-16 max-w-325 mx-auto grid gap-32 items-center"
        style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}
      >
        <div>
          <Reveal><SectionLabel>About</SectionLabel></Reveal>
          <Reveal delay={0.1}>
            <h2 className="heading-font font-light leading-none tracking-[-0.02em] mb-8 text-[clamp(3rem,5vw,5rem)]">
              Crafting<br />
          ...
          <Reveal delay={0.2}>
            <p className="text-(--muted) text-[1.05rem] leading-[1.9] mb-6">
            ...
            <p className="text-(--muted) text-[1.05rem] leading-[1.9]">
          ...
          <Reveal delay={0.3}>
            <div className="flex gap-12 mt-12 pt-12 border-t border-(--border)">
              {[['4+', 'Years Experience'], ['10+', 'Projects Shipped'], ['3+', 'Industries Served']].map(([num, label]) => (
                <div key={label}>
                  <div className="heading-font text-[3.5rem] font-light text-gold leading-none">{num}</div>
                  <div className="font-mono text-[0.72rem] tracking-[0.12em] uppercase text-(--muted) mt-1">{label}</div>
                </div>
              ))}
            </div>
          </Reveal>
        ...
        <Reveal delay={0.15}>
          ...
          <div className="border border-(--border) grid grid-cols-2">
            {skills.map((skill, i) => (
              <div
                key={skill}
                className={`skill-item ${i % 2 === 0 ? 'border-r border-(--border)' : ''} ${i < skills.length - 2 ? 'border-b border-(--border)' : ''}`}
              >
      ...
      {/* ══════════════ PROJECTS ══════════════ */}
      <section id="projects" className="py-32 px-16 bg-(--ink2)">
        <div className="max-w-325 mx-auto">
          <div className="flex items-end justify-between mb-20 flex-wrap gap-6">
            <div>
              ...
              <Reveal delay={0.1}>
                <h2 className="heading-font font-light leading-none tracking-[-0.02em] text-[clamp(2.5rem,5vw,4rem)]">
          ...
          <Reveal>
            <div
              className="project-card mb-0.75"
            ...
            >
              ...
              <div
                ...
                style={{
                  width: hoveredProject === 0 ? 160 : 100,
                  height: hoveredProject === 0 ? 160 : 100,
                }}
              />
              <div className="absolute bottom-0 right-0 w-15 h-15 border-b border-r border-[rgba(201,168,76,0.2)]" />
              <div className="absolute bottom-0 left-0 right-0 p-12">
                <p className="font-mono text-[0.68rem] tracking-[0.2em] uppercase text-gold mb-3">
                  {projects[0].tag}
                </p>
                <p className="text-[1rem] text-[rgba(245,240,232,0.78)] leading-[1.85] mb-5 max-w-170">
                  {projects[0].desc}
                </p>
                <p className="project-stack-text font-mono text-[0.72rem] text-(--muted) tracking-[0.08em]">
                  {projects[0].stack}
                </p>
              </div>
            </div>
      ...
      {/* ══════════════ EXPERIENCE ══════════════ */}
      <section id="experience" className="py-40 px-16 bg-(--ink)">
        <div className="max-w-325 mx-auto">
          <Reveal><SectionLabel>Experience</SectionLabel></Reveal>
          <Reveal delay={0.1}>
            <h2 className="heading-font font-light leading-none tracking-[-0.02em] mb-20 text-[clamp(3rem,5vw,5rem)]">
          ...
          {experience.map((e, i) => (
            <Reveal key={e.role} delay={i * 0.1}>
              <div className="exp-item grid gap-16" style={{ gridTemplateColumns: '200px 1fr' }}>
                <div>
                  <div className="font-mono text-[0.75rem] text-gold tracking-widest mb-1">{e.period}</div>
                  <div className="text-[0.85rem] text-(--muted)">{e.company}</div>
                </div>
                <div>
                  <div className="exp-role-title heading-font text-[1.8rem] font-light leading-[1.2] mb-4">{e.role}</div>
                  <div className="text-[0.92rem] text-(--muted) leading-[1.8]">{e.desc}</div>
                  <div className="flex flex-wrap gap-2 mt-6">
                    {e.tags.map(tag => (
                      <span
                        key={tag}
                        className="font-mono text-[0.68rem] tracking-widest uppercase px-3 py-1 border border-(--border) text-(--muted) rounded-xs"
                      >
                        {tag}
                      </span>
                    ))}
      ...
      {/* ══════════════ CONTACT ══════════════ */}
      <section id="contact" className="py-40 px-16 bg-(--ink2) text-center relative overflow-hidden">
        <div className="absolute inset-0 contact-radial-bg" />
        {/* Watermark */}
        <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 heading-font font-light text-[rgba(201,168,76,0.03)] tracking-[-0.05em] whitespace-nowrap leading-none pointer-events-none select-none text-[clamp(8rem,20vw,18rem)]">
          Contact
        </div>

        <div className="relative z-2 max-w-200 mx-auto">
          <Reveal>
          ...
          <Reveal delay={0.1}>
            <h2 className="heading-font font-light leading-[0.95] tracking-[-0.02em] mb-8 text-[clamp(3.5rem,8vw,7rem)]">
              Let&apos;s Build<br />
              <em className="italic text-gold">Together</em>
            </h2>
          </Reveal>
          <Reveal delay={0.2}>
            <p className="text-[1rem] text-(--muted) mb-14 leading-[1.9]">
          ...
          <Reveal delay={0.25}>
            <a
              ...
              className="contact-email text-[clamp(1rem,2.2vw,1.8rem)] mb-16 inline-flex"
            >
          ...
          <Reveal delay={0.35}>
            <div className="pt-12 border-t border-(--border) font-mono text-[0.75rem] tracking-[0.15em] uppercase text-(--muted)">
      ...
      {/* ══════════════ FOOTER ══════════════ */}
      <footer className="py-8 px-16 flex items-center justify-between border-t border-(--border) bg-(--ink)">
        <div className="font-mono text-[0.7rem] text-(--muted) tracking-widest">
          © 2026 Rahul Yadav. All rights reserved.
        </div>
        <button
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="font-mono text-[0.7rem] tracking-widest uppercase text-(--muted) bg-transparent border-none cursor-none transition-colors duration-300 hover:text-gold"
        >
          Back to top ↑
        </button>
      </footer>
